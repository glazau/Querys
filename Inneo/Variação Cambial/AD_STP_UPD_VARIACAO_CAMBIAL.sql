...
*/
SELECT NUFIN,VLRMOEDA,CODTIPOPERBAIXA,CODPROJ,DTREF,VLRDESDOB,TIPO,TIPON,VLRUS,AD_CALCVARCAMB,VARCALC,CODMOEDA,VLRMOEDABAIXA,VLRVARCAMBIAL,VLRBAIXA
FROM (
/*
...
SELECT FIN.NUFIN,NVL(FIN.VLRMOEDA,0) AS VLRMOEDA,FIN.CODTIPOPERBAIXA,FIN.CODPROJ,PARAM_DATAREF  AS "DTREF" ,FIN.VLRDESDOB,
	   ...
	   '1' AS TIPON,'N' AS "AD_CALCVARCAMB",CAMB.VARCALC,MAX(DTVAR)  AS "CAMBDTVAR",FIN.CODMOEDA,FIN.VLRMOEDABAIXA,FIN.VLRVARCAMBIAL,FIN.VLRBAIXA
FROM TGFFIN FIN
...
GROUP BY FIN.NUFIN,FIN.VLRMOEDA,FIN.CODTIPOPERBAIXA,FIN.CODPROJ,FIN.DTNEG,FIN.VLRDESDOB,CAMB.VARCALC,FIN.CODMOEDA,FIN.VLRMOEDABAIXA,FIN.VLRVARCAMBIAL,FIN.VLRBAIXA
...
SELECT FIN.NUFIN,NVL(FIN.VLRMOEDA,0) as VLRMOEDA,FIN.CODTIPOPERBAIXA,FIN.CODPROJ,FIN.DHBAIXA as DTREF,FIN.VLRDESDOB,
	   ...
	   '3' AS "TIPON",NVL(FIN.AD_CALCVARCAMB,'N') AS "AD_CALCVARCAMB",CAMB.VARCALC,MAX(CAMB.DTVAR) AS "CAMBDTVAR",FIN.CODMOEDA,FIN.VLRMOEDABAIXA,FIN.VLRVARCAMBIAL,FIN.VLRBAIXA
FROM TGFFIN FIN
INNER JOIN TGFNAT NAT ON NAT.CODNAT = FIN.CODNAT
INNER JOIN TGFTOP TPO ON TPO.CODTIPOPER = FIN.CODTIPOPER AND TPO.DHALTER = FIN.DHTIPOPER
...
GROUP BY FIN.NUFIN,FIN.VLRMOEDA,FIN.CODTIPOPERBAIXA,FIN.CODPROJ,FIN.DTNEG,FIN.DHBAIXA,FIN.AD_CALCVARCAMB,FIN.VLRDESDOB,CAMB.VARCALC,FIN.CODMOEDA,FIN.VLRMOEDABAIXA,FIN.VLRVARCAMBIAL,FIN.VLRBAIXA
...
SELECT DISTINCT FIN.NUFIN,NVL(FIN.VLRMOEDA,0) as VLRMOEDA,FIN.CODTIPOPERBAIXA,FIN.CODPROJ, MAX(CAB.DTNEG) AS DTREF,FIN.VLRDESDOB,
				...
				'2' AS TIPON,'N' AS "AD_CALCVARCAMB",CAMB.VARCALC,MAX(DTVAR)  AS "CAMBDTVAR",FIN.CODMOEDA,FIN.VLRMOEDABAIXA,FIN.VLRVARCAMBIAL,FIN.VLRBAIXA 
FROM TGFFIN FIN 
...
GROUP BY FIN.NUFIN,FIN.VLRMOEDA,FIN.CODTIPOPERBAIXA,FIN.CODPROJ,FIN.VLRDESDOB,CAMB.VARCALC,FIN.CODMOEDA,FIN.VLRMOEDABAIXA,FIN.VLRVARCAMBIAL,FIN.VLRBAIXA
) T 
WHERE 1=1    
...
LOOP   
	...
	/*
	Necessário preencher o cod.moeda para calcular os eventos.
	*/
	IF (CUR.CODMOEDA IS NULL OR CUR.CODMOEDA = 0) THEN
	RAISE_APPLICATION_ERROR(-20101, 'Favor preencher o campo Cod. Moeda do Título Financeiro, não é possível calcular a variação cambial com o cod.moeda zerado');
	END IF;
    ...
    */
    ...
    /*
    INSERT NACIONALIZAÇÃO ANTES DA BAIXA
    */
    IF CUR.TIPON = '2' AND (CUR.AD_CALCVARCAMB = 'N' OR CUR.AD_CALCVARCAMB IS NULL) AND (CUR.VARCALC = 'N' OR CUR.VARCALC IS NULL) AND P_TEM_REG_BAIXADO = 0 THEN
	INSERT INTO AD_VARCAMB (NUFIN,SEQUENCIA,DTVAR,DTINC,VLRMOEDA,VLRMOEDAVAR,VLRMODFUN,VLRMODFUNATUAL,VLRMODUS,VLRVARCAMB,VLRVARCAMBLANC,TIPO,VARCALC)
	...
	END IF;
	   ...
	   /*
    INSERT NACIONALIZAÇÃO APÓS BAIXA
    */
    IF CUR.TIPON = '2' AND (CUR.AD_CALCVARCAMB = 'N' OR CUR.AD_CALCVARCAMB IS NULL) AND (CUR.VARCALC = 'N' OR CUR.VARCALC IS NULL) AND P_TEM_REG_BAIXADO > 0 THEN
	INSERT INTO AD_VARCAMB (NUFIN,SEQUENCIA,DTVAR,DTINC,VLRMOEDA,VLRMOEDAVAR,VLRMODFUN,VLRMODFUNATUAL,VLRMODUS,VLRVARCAMB,VLRVARCAMBLANC,TIPO,VARCALC)
	VALUES(FIELD_NUFIN,(SELECT NVL(MAX(SEQUENCIA)+1,1) FROM AD_VARCAMB WHERE NUFIN = FIELD_NUFIN),CUR.DTREF,SYSDATE,CUR.VLRMOEDA,V_COTACAO,CUR.VLRDESDOB,CUR.VLRUS*V_COTACAO,CUR.VLRDESDOB/CUR.VLRMOEDA,(CUR.VLRUS*V_COTACAO)-CUR.VLRDESDOB,(((CUR.VLRUS*V_COTACAO)-CUR.VLRDESDOB) - NVL(V_VLRVARCAMBLANC,0)),CUR.TIPON,'S');
	...
	*/	  
	...
	INSERT INTO AD_VARCAMB (NUFIN,SEQUENCIA,DTVAR,DTINC,VLRMOEDA,
	VLRMOEDAVAR,VLRMODFUN,VLRMODFUNATUAL,VLRMODUS,VLRVARCAMB,VLRVARCAMBLANC,TIPO,VARCALC)
	VALUES(FIELD_NUFIN,(SELECT NVL(MAX(SEQUENCIA)+1,1) FROM AD_VARCAMB WHERE NUFIN = FIELD_NUFIN),CUR.DTREF,SYSDATE,CUR.VLRMOEDA,
	CUR.VLRMOEDABAIXA,CUR.VLRDESDOB,CUR.VLRUS*CUR.VLRMOEDABAIXA,CUR.VLRDESDOB/CUR.VLRMOEDA,(CUR.VLRUS*CUR.VLRMOEDABAIXA)-CUR.VLRDESDOB,(((CUR.VLRUS*CUR.VLRMOEDABAIXA)-CUR.VLRDESDOB) - NVL(V_VLRVARCAMBLANC,0)),CUR.TIPON,'S');
